...
- **`Outscripts/`** — 修改后脚本的导出目录（保持与源码相同的目录结构）。当前已包含修改文件：FighterMcCtrler.as、ConfigVO.as、FighterAILogic.as、MenuBtnGroup.as、GameInterfaceManager.as、SetCtrlBtnUI.as

## 交流语言

**所有对话必须使用中文**（技术术语、代码标识符、文件路径等可保留英文），技术播报（如文件列表、git 状态等）除外。
...
│   │   │   ├── FighterVO.as           # 角色定义（id、名称、comicType、文件、BGM、头像等）
...
│   │   │   ├── FighterHitRange.as     # 攻击范围定义（kanmian/bsmian/kbsmian 等常量）
│   │   │   ├── FighterDefenseType.as  # 防御类型枚举（SWOARD=0 刀, HAND=1 拳）
│   │   │   ├── Assister.as            # 辅助角色实体
│   │   │   ├── Bullet.as              # 飞行道具实体
│   │   │   ├── ctrler/                # 角色控制器
...
│   │   │   ├── utils/McAreaCacher.as         # 按帧缓存攻击/身体/hit_check 判定区域
│   │   │   └── vos/
...
│   │   │   ├── SetBtn.as / SetBtnDialog.as / SetBtnGroup.as / SetBtnLine.as  # 按钮设置（另有 SetBtnGroup.pcode 文件）
...
└── [根目录嵌入资源 .as 文件及 _assets/ 目录]
    ├── *_png$*.as / §*_png$*§.as       # 嵌入的 PNG 位图
    ├── *_swf$*.as / §*_swf$*§.as       # 嵌入的 SWF MovieClip
    ├── *ByteArray.as / §*ByteArray§.as  # SWF 二进制数据伴生文件
    ├── snd_*.as                         # 嵌入的声音文件
    ├── script_NNN.as                    # 嵌入 MovieClip 的时间轴脚本
    └── _assets/                         # 嵌入资源的 .bin 二进制数据文件
```
  ...
  → initlize()（addedToStage 事件触发）
    ...
    → UIAssetUtil.I.initalize → buildGame()
      → MainGame.initlize()
        ...
        → GameInputer.updateConfig（更新输入配置）
...
#### FighterVO 关键属性（角色配置数据）

| 属性 | 类型 | 说明 |
|------|------|------|
| `id` | String | 角色唯一标识 |
| `name` | String | 角色名称 |
| `comicType` | int | 漫画类型（1=死神, 其他=火影等） |
| `fileUrl` | String | SWF 文件路径 |
| `startFrame` | int | 起始帧 |
| `faceUrl` | String | 头像 URL |
| `faceBigUrl` | String | 大头像 URL |
| `faceBarUrl` | String | 血条头像 URL |
| `faceWinUrl` | String | 胜利画面头像 URL |
| `bgm` | String | 背景音乐 URL |
| `bgmRate` | Number | BGM 播放速率 |
| `contactFriends` | Array | 友方互动角色 ID 列表 |
| `contactEnemys` | Array | 敌方互动角色 ID 列表 |
| `says` | Array | 台词列表 |
...
| `speed`              | Number            | 移动速度（默认 6）                    |
| `jumpPower`          | Number            | 跳跃力度（默认 15）                   |
| `heavy`              | Number            | 重量（影响击飞距离，默认 2）          |
| `actionState`        | int               | 当前动作状态（见 FighterActionState） |
| `defenseType`        | int               | 防御类型（0=SWOARD 刀, 1=HAND 拳）    |
| `airHitTimes`        | int               | 空中可攻击次数                        |
| `jumpTimes`          | int               | 可跳跃次数（默认 2）                  |
| `isSteelBody`        | Boolean           | 是否钢身状态                          |
| `isSuperSteelBody`   | Boolean           | 是否超级钢身                          |
| `data`               | FighterVO         | 角色配置数据                          |
| `direct`             | int               | 朝向（1=右，-1=左）                   |
| `isCross`            | Boolean           | 是否可穿越对手                        |
| `isAllowBeHit`       | Boolean           | 是否可被攻击                          |
| `targetTeams`        | Vector.\<TeamVO\> | 敌对队伍列表                          |
| `energyOverLoad`     | Boolean           | 体力是否过载（影响体力恢复速度）      |
| `customHpMax`        | int               | 自定义 HP 上限（0=使用默认 1000）     |

#### 动作状态常量（FighterActionState）

> **注意**：反编译代码中常量名为 `NORNAL`（拼写错误），实际值为 0，语义等同于"normal/站立"。

```
NORNAL = 0           普通/站立（代码中拼写为 NORNAL）
...
- `allowGhostStep(state)` → 非必杀且非万解（state ∉ [12, 13, 50]）
- `isAllowWinState(state)` → 非必杀且非万解（state ∉ [12, 13, 50]）
...
| 类别 | 属性 | 默认值 | 说明 |
|------|------|--------|------|
| 气量消耗 | `bishaQi` | 100 | 必杀气量消耗 |
| | `bishaUPQi` | 100 | 上必杀气量消耗 |
| | `bishaSUPERQi` | 300 | 超必杀气量消耗 |
| | `bishaAIRQi` | 100 | 空中必杀气量消耗 |
| 动作帧标签 | `attack`、`skill1`、`skill2` | null | 地面攻击动作帧标签 |
| | `zhao1`、`zhao2`、`zhao3` | null | 招式动作帧标签 |
| | `bisha`、`bishaUP`、`bishaSUPER` | null | 必杀动作帧标签 |
| | `catch1`、`catch2` | null | 投技动作帧标签 |
| | `attackAIR`、`skillAIR`、`bishaAIR` | null | 空中攻击动作帧标签 |
| | `waiKai`、`waiKaiW`、`waiKaiS` | null | 万解动作帧标签 |
| 状态标志 | `isMoving`、`isJumping`、`isDefensing` | false | 状态标记 |
| | `isDashing`、`isHurting`、`isHurtFlying` | false | 状态标记 |
| CD 系统 | `setCD(key, frame)` | — | 设置冷却（帧数会自动除以 FPS_GAME） |
| | `CDOK(key)` | — | 检查冷却是否完成 |

**动作状态优先级**（地面，按代码检查顺序）：

```
catch1 → catch2 → 超必杀 → 上必杀 → 必杀 → 技能2 → 技能1 → 招3 → 招2 → 攻击 → 招1
→ 防御 → 瞬步 → 移动 → 跳跃 → 下落
→ 幽灵步/幽灵跳跃（仅在状态允许幽灵步时）
→ [若正在攻击状态] 跳砍/跳招/空中必杀
```
...
```
幽灵步/幽灵跳跃/幽灵下落 → 跳跃 → 下落 → 跳砍/跳招/空中必杀（仅在攻击状态中）
```

**幽灵步（Ghost Step）**：

- 通用机制，消耗 60 气 + 80 体力
- 三种类型（`_ghostType`）：0=水平幽灵步（移速 8）、1=幽灵跳跃（移速 -12）、2=幽灵下落（移速 15）
- 持续时间：`_ghostStepFrame = 30 * 0.4 = 12` 帧
...
| `SPEED_PLUS` | `30/FPS_GAME` | 帧归一化因子（getter 动态计算） |
| `FIGHTER_HP_MAX` | 1000 | 默认最大生命值 |
| `GAME_SIZE` | (800,600) | 游戏画面尺寸 |
| `JUMP_DELAY_FRAME` | 2 | 地面跳跃延迟帧 |
| `JUMP_DELAY_FRAME_AIR` | 1 | 空中跳跃延迟帧 |
| `HURT_DAMPING_X` | 0.1 | 受击 X 轴阻尼 |
| `HURT_DAMPING_Y` | 0.5 | 受击 Y 轴阻尼 |
| `HURT_FLY_DAMPING_X` | 0 | 击飞 X 轴阻尼 |
| `HURT_FLY_DAMPING_Y` | 0.5 | 击飞 Y 轴阻尼 |
| `HIT_FLOOR_DAMPING_X` | 2 | 普通落地 X 轴阻尼 |
| `HIT_FLOOR_DAMPING_X_HEAVY` | 4 | 重击落地 X 轴阻尼 |
| `HIT_FLOOR_TAN_Y_MIN/MAX` | 3/8 | 弹地 Y 轴速度范围 |
| `DEFENSE_DAMPING_X` | 1 | 防御 X 轴阻尼 |
| `DEFENSE_LOSE_HP_RATE` | 0.05 | 防御时扣血比例 |
| `HIT_DOWN_FRAME` | 15 | 普通倒地帧数 |
| `HIT_DOWN_FRAME_HEAVY` | 30 | 重击倒地帧数 |
| `STEEL_HURT_HP_PERCENT` | 0.65 | 钢身受击扣血比例 |
| `STEEL_SUPER_HURT_HP_PERCENT` | 0.3 | 超级钢身受击扣血比例 |
| `USE_ENERGY_CD` | 0.8 | 体力消耗冷却时间 |
| `ENERGY_ADD_NORMAL` | 2 | 普通状态体力恢复速度 |
| `QI_ADD_HIT_RATE` | 0.17 | 命中时气量获取比例 |
| `QI_ADD_HURT_RATE` | 0.08 | 受击时气量获取比例 |
| `CAMERA_TWEEN_SPD` | 2.5 | 摄像机补间速度 |
| `X_SIDE_OFFSET` | 10 | 边界偏移量 |
...
| `KAN` | 1 | 砍击 |
| `DA` | 2 | 打击 |
| `DA_HEAVY` | 3 | 重打击（`isHeavy`） |
| `MAGIC` | 4 | 魔法 |
| `MAGIC_HEAVY` | 5 | 重魔法（`isHeavy`） |
| `KAN_HEAVY` | 6 | 重砍击（`isHeavy`） |
| `FIRE` | 7 | 火焰（`isHeavy`） |
| `ICE` | 8 | 冰冻（`isHeavy`） |
| `ELECTRIC` | 9 | 雷电（`isHeavy`） |
| `STONE` | 10 | 石化 |
| `CATCH` | 11 | 投技（破防） |

**攻击范围常量（FighterHitRange）**：
| 常量 | 值 | 说明 |
|------|----|------|
| `ATTACK` | `"kanmian"` | 普通攻击范围 |
| `SKILL1` | `"kj1mian"` | 技能1范围 |
| `SKILL2` | `"kj2mian"` | 技能2范围 |
| `ZHAO1` | `"zh1mian"` | 招1范围 |
| `ZHAO2` | `"zh2mian"` | 招2范围 |
| `ZHAO3` | `"zh3mian"` | 招3范围 |
| `BISHA` | `"bsmian"` | 必杀范围 |
| `BISHA_UP` | `"sbsmian"` | 上必杀范围 |
| `BISHA_SUPER` | `"cbsmian"` | 超必杀范围 |
| `BISHA_AIR` | `"kbsmian"` | 空中必杀范围 |
| `JUMP_ATTACK` | `"tkanmian"` | 跳砍范围 |
| `JUMP_SKILL` | `"tzmian"` | 跳招范围 |

**必杀判断**：`isBisha()` 检查 id 是否包含 `bs`/`sbs`/`cbs`/`kbs`
**投技判断**：`isCatch()` = hitType==11（破防投技）且 isBreakDef
**重击判断**：`HitType.isHeavy(type)` → type 在 [3, 5, 6, 7, 8, 9]

### 7. AI 系统

`FighterAICtrl` + `FighterAILogic` — AI 实现与键盘输入完全相同的 `IFighterActionCtrl` 接口。AI 难度由 `MessionModel.I.AI_LEVEL` 控制。

FighterAILogic 包含所有 AI 行为布尔标志位：`moveLeft`、`moveRight`、`jump`、`jumpDown`、`downJump`、`dash`、`defense`、`attack`、`attackAIR`、`skillAIR`、`bishaAIR`、`skill1`、`skill2`、`zhao1`、`zhao2`、`zhao3`、`catch1`、`catch2`、`bisha`、`bishaUP`、`bishaSUPER`、`assist`、`specialSkill`、`ghostStep`、`ghostJump`、`ghostJumpDowm`。
...
#### 7.2 AI 基类 (FighterAILogicBase)

`FighterAILogic` 继承自 `FighterAILogicBase`，基类提供：
- `getTarget()` — 获取 AI 目标
- `getTargetArea()` — 获取目标身体区域
- `canHit()` / `canHitOnAi()` — 预判攻击是否能命中
- `targetInRange()` — 目标是否在范围内
- `targetInZhaoRange()` — 目标是否在招式范围内
- `targetCanBeHit()` — 目标是否可被命中
- `getBishaAI()` — 必杀决策核心函数
- `getDistance()` / `getAirDistance()` — 距离计算

### 7.3 无限气系统 (INFINITE_ENERGY)

三值联动控制：`GameConfig.INFINITE_ENERGY` (静态) + `ConfigVO.INFINITE_ENERGY` (实例) + `GameInterfaceManager.config.INFINITE_ENERGY` (移动端配置)。

**同步路径**:
| 操作 | ConfigVO | GameConfig | GameInterfaceManager | 同步方式 |
|------|:---:|:---:|:---:|------|
...
| 运行时同步 | - | - | - | `FighterAction.render()` 每帧从 `GameInterfaceManager.config` 同步 qi 消耗值 |

**影响范围**:
- `setBisha/setBishaUP/setBishaSUPER/setBishaAIR`: 控制 `_action.bishaQi` (0 或默认消耗)
- `FighterAction.render()`: **每帧**从 `GameInterfaceManager.config.INFINITE_ENERGY` 读取，同步重置各必杀气量消耗为默认值（100/300）
...
- `dash()` — 瞬步
- `defense()` — 防御
- `ghostStep()`、`ghostJump()`、`ghostJumpDown()` — 幽灵步（水平/跳跃/下落）
- `waiKai()`、`waiKaiW()`、`waiKaiS()` — 万解
- `assist()` — 召唤辅助
- `attackAIR()`、`skillAIR()`、`bishaAIR()` — 空中攻击/技能/必杀
...
- `isTeamDuo()` → mode 在 [14, 15]（双人组队）
- `isDuoMode()` → mode 在 [24, 25]（一打二）
...
- **文件名特殊字符**：`§` 包裹部分嵌入资源文件名（如 `§gameover_swf$hash§.as`），常见于 Flash 编译器特殊嵌入；`$` 分隔资源名和哈希值（`name$hash.as`）
