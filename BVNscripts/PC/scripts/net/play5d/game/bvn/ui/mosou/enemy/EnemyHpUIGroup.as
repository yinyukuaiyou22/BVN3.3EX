package net.play5d.game.bvn.ui.mosou.enemy
{
   ...
   public class EnemyHpUIGroup
   {
      ...
      public function render() : void
      {
         var _loc3_:int = 0;
         var _loc2_:* = null;
         var _loc1_:Vector.<EnemyHpUI> = new Vector.<EnemyHpUI>();
         _loc3_ = 0;
         while(_loc3_ < _uis.length)
         {
            if(!_uis[_loc3_].render())
            {
               _loc1_.push(_uis[_loc3_]);
            }
            _loc3_++;
         }
         if(_loc1_.length > 0)
      ...
      private function sortUI() : void
      {
         ...
         var _loc1_:int = 0;
         var _loc4_:int = 0;
         var _loc2_:int = Math.min(_uis.length,12);
         _ct.removeChildren();
         _loc4_ = 0;
         while(_loc4_ < _loc2_)
         {
            _uis[_loc4_].getUI().x = _loc3_;
            _uis[_loc4_].getUI().y = _loc1_;
            _loc3_ -= 110;
            if((_loc4_ + 1) % 4 == 0)
            {
               _loc3_ = 0;
