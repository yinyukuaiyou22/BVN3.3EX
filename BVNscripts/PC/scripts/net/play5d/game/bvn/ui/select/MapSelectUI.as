package net.play5d.game.bvn.ui.select
{
   ...
   public class MapSelectUI extends Sprite
   {
      ...
      {
         _mapmc = AssetManager.I.createObject("select_map_mc","subswfs/select.swf") as MovieClip;
         _maps = MapModel.I.getAllMaps();
         _mapmc.x = (GameConfig.GAME_SIZE.x - _mapmc.bg.width) * 0.5;
         _mapmc.y = GameConfig.GAME_SIZE.y * 0.25;
         _mapmc.ct.x -= 2;
         addChild(_mapmc);
         if(GameUI.SHOW_CN_TEXT)
         {
            _txtmc = AssetManager.I.createObject("select_map_txt_mc","subswfs/select.swf") as MovieClip;
            _txtmc.x = (GameConfig.GAME_SIZE.x - _txtmc.width) * 0.5;
      ...
      {
         var _loc3_:MapVO = _maps[_curId];
         var _loc2_:Boolean = false;
         if(_loc3_)
         {
            while(_loc3_.id == "random")
            {
               _loc3_ = KyoRandom.getRandomInArray(_maps);
               _loc2_ = true;
            }
            if(GameData.I.config.isStandardLimit && _loc2_)
            {
               showMap(_maps.indexOf(_loc3_));
               return;
            }
            GameData.I.selectMap = _loc3_.id;
            enabled = false;
            if(param1 != null)
            {
               param1();
      ...
      public function showRandomMap() : Boolean
      {
         if(_maps[_curId].id == "random")
         {
            return false;
         }
         showMap(0);
