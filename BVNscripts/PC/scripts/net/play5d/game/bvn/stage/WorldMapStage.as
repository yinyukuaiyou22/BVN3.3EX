package net.play5d.game.bvn.stage
{
   ...
   public class WorldMapStage implements Istage
   {
      ...
      {
         _ui = new Sprite();
         var _loc1_:Class = AssetManager.I.getSWFEffectClass(ResUtils.BIG_MAP,"subswfs/bigmap.swf");
         _mapUI = new _loc1_() as MovieClip;
         _ui.addChild(_mapUI);
         ...
         _backBtn = AssetManager.I.createObject("backbtn_btn","subswfs/dialog_ui.swf") as DisplayObject;
         if(_backBtn)
         {
            _backBtn.y = 10;
            _backBtn.x = 10;
      ...
      {
         for each(var _loc2_ in _pointUIs)
         {
            _loc2_.update();
         }
      }
      ...
      private function initPointUIs() : void
      {
         ...
         var _loc3_:int = 0;
         var _loc6_:WorldMapPointUI = null;
         var _loc2_:Object = {};
         var _loc4_:Object = {};
         _loc3_ = 0;
         while(_loc3_ < _pointMc.numChildren)
         {
            _loc8_ = _pointMc.getChildAt(_loc3_);
            if(_loc8_ != null)
            {
               _loc2_[_loc8_.name] = _loc8_;
               _loc8_.visible = false;
            }
            _loc3_++;
         }
         _loc3_ = 0;
         while(_loc3_ < _maskMc.numChildren)
         {
            _loc8_ = _maskMc.getChildAt(_loc3_);
            if(_loc8_ != null)
            {
         ...
         _loc7_ = MosouModel.I.getMap(GameData.I.mosouData.getCurrentMap().id);
         var _loc5_:Vector.<MosouWorldMapAreaVO> = _loc7_.areas;
         for each(var _loc1_ in _loc5_)
         {
            if(_loc2_[_loc1_.id])
            {
               _loc6_ = new WorldMapPointUI(_loc2_[_loc1_.id],_loc4_[_loc1_.id],_loc1_);
               _loc6_.addEventListener("EVENT_SELECT",onSelectPoint);
               _pointUIs.push(_loc6_);
            }
         }
      }
      ...
      private function onSelectPoint(param1:Event) : void
      {
         ...
         if(_loc3_.building())
         {
            GameUI.alert(GetLangText("game_ui.alert.musou_not_open.title"),GetLangText("game_ui.alert.musou_not_open.message"),null,true);
            return;
         }
      ...
      private function gotoMission(param1:MosouWorldMapAreaVO) : void
      {
         ...
         if(allFinish)
         {
            missionNum = int(data.missions.length);
            ...
            {
               GameUI.branch(GetLangText("game_ui.confrim.musou_confrim.title"),GetLangText("game_ui.confrim.musou_confrim.message_all_finish_yes"),function(param1:int):void
               {
                  MosouModel.I.currentMission = data.missions[param1 - 1];
                  StateCtrl.I.transIn(MainGame.I.loadGame,true);
               },null,missionNum);
            }
            return;
         }
         GameUI.confrim(GetLangText("game_ui.confrim.musou_confrim.title"),GetLangText("game_ui.confrim.musou_confrim.message_all_finish_no"),function():void
         {
      ...
      {
         var _loc1_:WorldMapPointUI = null;
         for each(var _loc3_ in _pointUIs)
         {
         ...
         var _loc2_:Point = new Point(GameConfig.GAME_SIZE.x * 0.5,GameConfig.GAME_SIZE.y * 0.5);
         _scale = 1.2;
      ...
      private function renderPosAndSize(param1:Boolean = false) : void
      {
         ...
         {
            _loc4_ = _mapUI.scaleX - _scale;
            if(_loc4_ != 0)
            {
               _position.x += _currentPoint.getPosition().x * _loc4_;
               _position.y += _currentPoint.getPosition().y * _loc4_;
            }
         }
         var _loc3_:Number = GameConfig.GAME_SIZE.x - _bgMc.width * _scale;
      ...
      {
         GameUI.confrim(GetLangText("game_ui.confrim.back_title.title"),GetLangText("game_ui.confrim.back_title.message"),MainGame.I.goMenu,null,true);
      }
