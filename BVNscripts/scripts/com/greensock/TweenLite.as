package com.greensock
{
   ...
   public class TweenLite extends TweenCore
   {
      ...
      public function TweenLite(target:Object, duration:Number, vars:Object)
      {
         ...
         this._overwrite = Number(vars.overwrite) <= -1 || !overwriteManager.enabled && vars.overwrite > 1 ? int(overwriteManager.mode) : int(vars.overwrite);
      ...
      protected function init() : void
      {
         ...
         for(p in this.vars)
         {
            if(!(p in _reservedProps && !(p == "timeScale" && this.target is TweenCore)))
            {
               ...
               {
                  this.cachedPT1 = new PropTween(plugin,"changeFactor",0,1,plugin.overwriteProps.length == 1 ? String(plugin.overwriteProps[0]) : "_MULTIPLE_",true,this.cachedPT1);
         ...
         if(Boolean(this._overwrite > 1 && this.cachedPT1) && (Boolean(siblings = masterList[this.target])) && siblings.length > 1)
         {
      ...
      override public function renderTime(time:Number, suppressEvents:Boolean = false, force:Boolean = false) : void
      {
         ...
         if(Boolean(prevTime == 0 && this.vars.onStart) && (Boolean(this.cachedTime != 0 || this.cachedDuration == 0)) && !suppressEvents)
         {
